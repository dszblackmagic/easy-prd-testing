...
6. 汇总执行记录、缺陷、阻塞和证据索引。
7. 在完整链路执行过程中维护用户可见的 task-list，让用户持续看到已完成、进行中、阻塞和剩余步骤。
...
- 将用户引导到最近的有效阶段。
- 阻止跳过规划确认、执行前门禁或风险授权。
- 在完整链路执行时维护用户可见 task-list，并在阶段切换、阻塞、恢复和完成时更新状态。
...
完整链路执行时，总入口必须维护一个用户可见的 task-list。它用于表达当前流程进度，不替代落盘产物。

推荐状态：

- `pending`：尚未开始。
- `in_progress`：当前正在处理。
- `blocked`：等待用户输入、环境、账号、权限、数据或依赖。
- `completed`：已完成。
- `skipped`：用户确认本轮不执行。
...
6. 等待用户显式确认 01-04
7. 确认测试环境地址和测试路径
8. 确认登录方式、账号、密码获取方式和权限范围
9. 确认执行模式和高风险动作授权
10. 执行 P0/P1/P2/P3 用例
11. 汇总执行记录和缺陷记录
12. 输出最终结论和证据索引
```
...
- 完整链路开始后，先给出初始 task-list。
- 每进入一个新阶段，更新当前任务为 `in_progress`，前一阶段标记为 `completed` 或 `blocked`。
- 每次向用户询问缺失项时，对应任务标记为 `blocked`，并说明等待的单个信息。
- 用户补齐信息后，恢复该任务并继续推进。
- 多 agent 执行时，`执行 P0/P1/P2/P3 用例` 可以展开为四个优先级子任务，但写入边界仍遵循多 agent 协议。
- task-list 是对话中的进度视图，不要求每次都写入文件；最终关键节点仍要落到 `05-执行记录.md`。

禁止事项：

- 不允许在没有实际完成时把任务标记为 `completed`。
- 不允许用 task-list 跳过显式确认门禁。
- 不允许把多个用户待确认项合并成一个泛泛的阻塞任务；仍然按阶段规则一次只问一个缺失项。
...
- `docs/workflow.md`：6 阶段工作流、门禁、阶段流转和用户可见 task-list 规则。
...
- 总入口 skill 描述了完整链路 task-list 的初始化、状态更新、阻塞恢复和完成规则。
...
9. 完整链路执行时有用户可见 task-list，能展示已完成、进行中、阻塞、跳过和剩余步骤。
10. 文档能解释安装、工作流、task-list、产物协议和多 agent 协议。

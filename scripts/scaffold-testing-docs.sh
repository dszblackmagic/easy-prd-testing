...
for priority in P0 P1 P2 P3; do
  ...
  mkdir -p \
    "$PRIORITY_DIR/screenshots" \
    "$PRIORITY_DIR/videos" \
    "$PRIORITY_DIR/scripts" \
    "$PRIORITY_DIR/regression-screenshots" \
    "$PRIORITY_DIR/regression-videos" \
    "$PRIORITY_DIR/regression-traces" \
    "$PRIORITY_DIR/regression-scripts"
  ...
  copy_if_missing "$ROOT_DIR/skills/regression-testing/templates/07-回归记录.md" "$PRIORITY_DIR/07-回归记录.md"
  copy_if_missing "$ROOT_DIR/skills/regression-testing/templates/08-回归缺陷状态.md" "$PRIORITY_DIR/08-回归缺陷状态.md"
done

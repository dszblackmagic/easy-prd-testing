...
- 在执行前自动推导可安全识别的信息，仅确认仍需用户补充或授权的地址、登录、账号、范围和风险事项。
...
调用原则很简单：完整测试默认调用 `easy-prd-testing`；只有独立分析需求、生成规划、导出 XMind 或执行回归时，才指定对应阶段。若当前安装形态单独暴露了阶段 Skill，可以直接调用；否则仍调用父 Skill 并在提示词中写明意图。
...
用户明确确认 `01-04` 后，`execution-gate` 会先从现有资料和只读探测中推导目标类型、服务状态与归属、页面就绪依据和清理责任，仅在无法可靠推导或确需授权时再逐项询问。已有本地服务默认保持运行，远程服务不会被自动启停；其他必要信息未解决前也不会直接执行。

### 4. 执行并保留证据

`test-execution` 默认使用 `agent-browser`，支持单 agent 或按 P0-P3 拆分执行。首次进入页面或关键流程时遵循“进入 → 就绪判断 → 页面侦察 → 定位 → 操作 → 断言 → 分类 → 证据 → 清理”，使用可观察的页面状态而非强制 `networkidle`；运行观测只记录工具实际产出的内容，并保持优先级目录与多 agent 写入边界不变。
...
> [!TIP]
> **v0.1.2 起支持整套 Skills 自更新。** 无需记忆固定命令，也不需要手动运行内部脚本，只要向 AI 表达 Easy PRD Testing 的升级意图即可。查看 [v0.1.2 更新说明](docs/releases/v0.1.2.md)。
